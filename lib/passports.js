...
module.exports = {
  init: (app) => {
      ...
      app.orm.Passport.findOne({
        ...
        .then(passport => {
          if (!req.user) {
            ...
            if (!passport) {
              app.services.FootprintService.create('user', user).then(user => {
                query.user = user.id
            ...
            // Action:   Get the user associated with the passport.
              ...
              app.services.FootprintService.update('Passport', passport.id, passport) .then(passport => {
                app.services.FootprintService.find('user', passport[0].user.id)
                  .then(user => {
                    next(null, user)
                  })
                  .catch(next)
    ...
    passport.deserializeUser((id, next) => {
      app.services.FootprintService.find('user', id).then(user => next(null, user))
    })
